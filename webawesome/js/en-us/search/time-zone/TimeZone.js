...
async function websocketTimeZone(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketTimeZone', function (error, message) {
      ...
      iTemplate.innerHTML = '<i class="{{ FONTAWESOME_STYLE }}fa-globe"></i>';
      var $i = iTemplate.content;
      var $headerSpan = document.createElement('span');
      $headerSpan.setAttribute('class', '');
      $headerSpan.innerText = 'modify time zones in ' + json.timeRemaining;
...
async function websocketTimeZoneInner(apiRequest) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.TimeZone_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.TimeZone_Page_archived');
        ...
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.TimeZone_Page_editPage');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.TimeZone_Page_userPage');
        ...
        if(inputCreated) {
          ...
          addGlow(document.querySelector('.TimeZone_Page_created'));
        }
        ...
        if(inputModified) {
          ...
          addGlow(document.querySelector('.TimeZone_Page_modified'));
        }
        ...
        if(inputArchived) {
          ...
          addGlow(document.querySelector('.TimeZone_Page_archived'));
        }
        ...
        if(inputAbbreviation) {
          ...
          addGlow(document.querySelector('.TimeZone_Page_abbreviation'));
        }
        ...
        if(inputEditPage) {
          ...
          addGlow(document.querySelector('.TimeZone_Page_editPage'));
        }
        ...
        if(inputUserPage) {
          ...
          addGlow(document.querySelector('.TimeZone_Page_userPage'));
        }
        ...
        if(inputDownload) {
          ...
          addGlow(document.querySelector('.TimeZone_Page_download'));
        }
        ...
        if(inputObjectTitle) {
          ...
          addGlow(document.querySelector('.TimeZone_Page_objectTitle'));
        }
...
function pageGraphTimeZone(apiRequest) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        if(range) {
          ...
          Plotly.react('htmBodyGraphTimeZonePage', data, layout);
        }
      }
    }
  }
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqTimeZone_time').innerText = '';
  searchPage('TimeZone', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqTimeZone_time').value = x;
      document.querySelector('#fqTimeZone_time').onchange();
      searchPage('TimeZone');
    }, speedRate);
  });
}
...
async function searchTimeZone($formFilters, success, error) {
  var filters = searchTimeZoneFilters($formFilters);
  if(success == null)
...
function searchTimeZoneFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function searchTimeZoneVals(filters, target, success, error) {
...
function suggestTimeZoneObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
      data['list'].forEach((o, i) => {
        var $i = document.querySelector('<i class="{{ FONTAWESOME_STYLE }}fa-globe"></i>');
  ...
  error = function( jqXhr, target2 ) {};
  searchTimeZoneVals($formFilters, target, success, error);
}
...
async function patchTimeZone($formFilters, $formValues, target, id, success, error) {
  var filters = patchTimeZoneFilters($formFilters);
...
function patchTimeZoneFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function patchTimeZoneVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchTimeZoneVals(filters, vals, target, success, error);
}

function patchTimeZoneVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/api/time-zone?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postTimeZone($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
...
function postTimeZoneVals(vals, target, success, error) {
  fetch(
    '/en-us/api/time-zone'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function deleteTimeZone(target, id, success, error) {
  ...
  fetch(
    '/en-us/api/time-zone/' + encodeURIComponent(id)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportTimeZone($formValues, target, id, success, error) {
  var json = $formValues.querySelector('.PUTImport_searchList')?.value;
  if(json != null && json !== '')
    putimportTimeZoneVals(JSON.parse(json), target, success, error);
}

function putimportTimeZoneVals(json, target, success, error) {
  fetch(
