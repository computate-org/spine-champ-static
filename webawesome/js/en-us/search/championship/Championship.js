...
async function websocketChampionship(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketChampionship', function (error, message) {
      ...
      iTemplate.innerHTML = '<i class="' + window.FONTAWESOME_STYLE + ' fa-court-sport"></i>';
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('correctGuessesChart'))
          inputCorrectGuessesChart = $response.querySelector('.Championship_Page_correctGuessesChart');
        ...
        if(inputCorrectGuessesChart) {
          document.querySelectorAll('.Championship_Page_correctGuessesChart').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
          ...
          addGlow(document.querySelector('.Championship_Page_correctGuessesChart'));
        }
...
function suggestChampionshipObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
      data['list'].forEach((o, i) => {
        var $i = document.querySelector('<i class="{{ FONTAWESOME_STYLE }} fa-court-sport"></i>');
        var $span = document.createElement('span');
        $span.setAttribute('class', '');
        $span.innerText = o['objectTitle'];
        var $li = document.createElement('li');
